...
from app.weixin import WXBizMsgCrypt
...
class Callback:
    ...
    def __init__(self):
        ...
        self.sNonce = 0
        self.wxcpt = WXBizMsgCrypt.WXBizMsgCrypt(self.config.get('WEIXIN_TOKEN'), self.config.get('WEIXIN_AESKEY'),
                                                 self.config.get('WEIXIN_CORPID'))
    ...
    def getScanResult(self, sMsg):
        xml_tree = ET.fromstring(sMsg)
        return xml_tree.find("ScanCodeInfo").find("ScanResult").text
    ...
    def mkQCdode(self, sToUser, scanResult):
        timestamp = str(int(time.time()))
        msg_plain = render_template('weixin/reply_qcode.xml',agentid=self.config.get('WEIXIN_AGTID'), fromUser=self.config.get('WEIXIN_CORPID'), toUser=sToUser,
                                    timestamp=timestamp, scanResult=scanResult)
        print(msg_plain)
        msg_encrypt = self.encode(msg_plain)
        return msg_encrypt

    def mkTextMsg(self, sToUser, sText):
        timestamp = str(int(time.time()))
        msg_plain = render_template('weixin/reply_text.xml', fromUser=self.config.get('WEIXIN_CORPID'), toUser=sToUser,
                                    timestamp=timestamp, content=sText)
        msg_encrypt = self.encode(msg_plain.encode('utf-8'))
        return msg_encrypt

    def mkNewsMsg(self, sToUser, arrArticles):
        timestamp = str(int(time.time()))
        msg_plain = render_template('weixin/reply_news.xml', fromUser=self.config.get('WEIXIN_CORPID'), toUser=sToUser,
                                    timestamp=timestamp, articles=arrArticles)
        msg_encrypt = self.encode(msg_plain.encode('utf-8'))
        return msg_encrypt
